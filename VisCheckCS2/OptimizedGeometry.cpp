...
bool OptimizedGeometry::CreateOptimizedFile(const std::string& rawFile, const std::string& optimizedFile) {
    // Используем Parser для извлечения чистых данных из .vphys файла
    Parser parser(rawFile);
    meshes = parser.GetCombinedList();

    std::ofstream out(optimizedFile, std::ios::binary);
    if (!out) {
        std::cerr << "Не удалось открыть файл для записи: " << optimizedFile << std::endl;
        return false;
    }
...
bool OptimizedGeometry::LoadFromFile(const std::string& optimizedFile) {
    ...
    if (!in) {
        std::cerr << "Не удалось открыть оптимизированный файл: " << optimizedFile << std::endl;
        return false;
    }
    meshes.clear();
    size_t numMeshes;
